...
fun InsulinAmountField(
    ...
    LaunchedEffect(state.text) {
        ...
        if (text.indexOfFirst { it == '.' } == -1) {
            text = when (text.length) {
                ...
                1 -> "0.0$text"
                2 -> "0.$text"
                else -> text.substring(0, text.length - 2) + "." + text.substring(text.length - 2)
            }
        } else if (text == ".")  {
            text = "0.0"
        }
    ...
    val largeTextStyle = MaterialTheme.typography.displayLarge.toSpanStyle()
    val smallTextStyle = MaterialTheme.typography.displaySmall.toSpanStyle()
